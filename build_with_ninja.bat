...
set PATH=%BLD_DIR%\execEnv\lib;%NINJADIR%;%TFDIR%;%QTDIR%\bin;%BOOST_LIBRARYDIR%;%MSYSDIR%;%WINKITDIR%;%PATH%
...
cmake -G "Ninja" "-DCMAKE_PREFIX_PATH=%QTDIR%;%BOOST_ROOT%" "-DLIMA_RESOURCES=build" "-DLIMA_VERSION_RELEASE:STRING=%LIMA_VERSION_RELEASE%" "-DTF_SOURCES_PATH=%TFINCLUDE%" -DCMAKE_BUILD_TYPE=RelWithDebInfo "-DCMAKE_INSTALL_PREFIX=%LIMA_DIST%" -DCMAKE_IGNORE_PATH=%MSYSDIR% %LIMA_SRC%
