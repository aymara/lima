...
std::pair<float, std::vector<torch::Tensor>> StanzaDepparseParserImpl::forward(
    torch::Tensor word,
    torch::Tensor word_mask,
    torch::Tensor wordchars,
    torch::Tensor wordchars_mask,
    torch::Tensor upos,
    torch::Tensor xpos,
    torch::Tensor ufeats,
    torch::Tensor pretrained,
    torch::Tensor lemma,
    torch::Tensor head,
    torch::Tensor deprels,
    torch::Tensor word_orig_idx,
    torch::Tensor sentlens,
    torch::Tensor wordlens)
{
...
// torch::Tensor input, torch::Tensor seqlens, torch::Tensor hx=torch::Tensor()
  ...
  if (is_training())
  {
      ...
      deprel_scores = torch::gather(deprel_scores, 2,
                                    head.unsqueeze(2).unsqueeze(3).expand({-1, -1, -1,
                                      (*m_vocab)["deprel"].size()})).view({-1, (*m_vocab)["deprel"].size()});
