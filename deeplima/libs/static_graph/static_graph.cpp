...
void StaticGraphImpl::prepare_exec_plan(
    ...
    )
{
  ...
  string key = get_exec_plan_key(inputs, outputs);
  if (m_exec_plans.end() != m_exec_plans.find(key))
  {
    throw;
  }

  set<size_t> given_inputs_idx;
  ...
  vector<size_t> temp;
  temp.reserve(m_ops.size() * outputs.size());

  list<size_t> required;
  //required.reserve(m_tensor_name_to_idx.size());
...
std::string StaticGraphImpl::get_exec_plan_key(
    const std::map<std::string, torch::Tensor>& inputs,
    const set<string>& outputs
    )
{
...
map<string, torch::Tensor> StaticGraphImpl::forward(
    const map<string, torch::Tensor>& inputs,
    const set<string>& outputs
    )
{
  assert(inputs.size() > 0);
  ...
  if (m_exec_plans.end() == m_exec_plans.find(key))
  {
    prepare_exec_plan(inputs, outputs);
  }

  const vector<size_t> plan = m_exec_plans[key];
