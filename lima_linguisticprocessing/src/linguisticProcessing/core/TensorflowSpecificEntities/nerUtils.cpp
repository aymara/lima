...
std::map<QString,int> loadFileWords(const QString& filepath)
{
    QFile file(filepath);
...
std::map<QChar,int> loadFileChars(const QString& filepath)
{
    QFile file(filepath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)){
      throw BadFileException("The file "+filepath.toStdString()+" doesn't exist.");
...
std::map<unsigned int,QString> loadFileTags(const QString& filepath)
{
    QFile file(filepath);
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)){
      throw BadFileException("The file "+filepath.toStdString()+" doesn't exist.");
...
NERStatusCode loadTextToEvaluate(
  QTextStream& qtIn, 
  QFile& file, 
...
std::pair<std::vector<int>,int> getProcessingWord(
