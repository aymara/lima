...
// antinno travaille avec la version 2.1-patches tant que la version master n'est pas synchronisée
  ...
  for (std::map<int,QString>::iterator it=sentences.begin(); it!=sentences.end(); ++it)
  {
    ...
    if(cHandler.extractSemanticInformation(sentenceIndex, limaConllMapping, sentence))
    {
      ...
      for (int vClassIndex=0;vClassIndex<cHandler.m_verbalClassNb;vClassIndex++)
      {
		...
		// Modif NAN compatibilité de compilation
		for (QStringList::iterator it=verbalClasses.begin(); it!=verbalClasses.end(); ++it)
		{
		  QString& verbalClass = *it;
        ...
        for (semRoleIt=cHandler.m_semanticRoles[vClassIndex].begin();         semRoleIt!=cHandler.m_semanticRoles[vClassIndex].end();semRoleIt++){
		  ...
		  // Modif NAN compatibilité de compilation
		  for (QStringList::iterator it=semanticRoles.begin(); it!=semanticRoles.end(); ++it)
          {
			  QString& semanticRole = *it;
...
#ifdef ANTINNO_SPECIFIC
BOOST_FOREACH (const auto & token, sentenceTokens)
#else
...
#ifdef ANTINNO_SPECIFIC
        BOOST_FOREACH (auto roleTargetFieldIndex, boost::irange(0,m_verbalClassNb))
#else
