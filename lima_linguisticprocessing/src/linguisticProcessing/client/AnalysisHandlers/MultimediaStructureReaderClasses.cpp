...
void MultimediaBinaryStructureReader::readBinNum(istream& file, uint64_t& cid, string& path, 
                                                       string& name, std::vector< float >& vect)
{
...
void MultimediaXMLStructureReader::closePropertiesNode(const Misc::GenericDocumentProperties* prop)
{
  ...
  typedef Misc::GenericDocumentProperties::IntPropertiesIterator IntIter;
  {
    ...
    for( IntIter it = ret.first ; it != ret.second ; it++ ) {
      ...
      if (elmt.first.compare("StructureId")==0)
        StructId=elmt.second ;
      else if (elmt.first.compare("offBegPrpty")==0)
        offBegPrpty=elmt.second ;
      else if (elmt.first.compare("offEndPrpty")==0)
        offEndPrpty=elmt.second ;
      else if (elmt.first.compare("NodeId")==0)
      {NodeId=elmt.second ;} 
    }
  }
  typedef Misc::GenericDocumentProperties::StringPropertiesIterator StringIter;
  {
    ...
    for( StringIter it = ret.first ; it != ret.second ; it++ ) {
      ...
      if (elmt.first.compare("encodPrpty")==0)
          encodPrpty=elmt.second ;
      else if (elmt.first.compare("langPrpty")==0)
          langPrpty=elmt.second ;
      else if (elmt.first.compare("srcePrpty")==0)
  ...
  typedef Misc::GenericDocumentProperties::DatePropertiesIterator DateIter;
  {
    ...
    for( DateIter it = ret.first ; it != ret.second ; it++ ) {
      ...
      if (elmt.first.compare("indexDatePrpty")==0)
...
//    cout<<"ici   "<<StructId<<"       "<<NodeId<<endl;
  ...
  std::map<uint64_t,Node>::const_iterator ItrNodes;
  for (ItrNodes=Nodes.begin();ItrNodes!=Nodes.end();ItrNodes++)
  {
    Node laNode=ItrNodes->second;
    string uri=laNode.uri();
    if (uri.length()!=0)
      srcePrpty=uri;
   ...
   Node node(MULTIMEDIA_DOCUMENT_READER0, StructId , NodeId, laNode.get_ContentId(),laNode.indexId(),
             srcePrpty , "","", laNode.descrId(), 0, 0, 0, 0,offBegPrpty, offEndPrpty, 
             offEndPrpty-offBegPrpty, m_lastNode);
...
void MultimediaBinaryStructureReader::readMultimediaDocumentBlock(std::istream& file,
...
// 	cout<<"ici ----1--- "<<handler->StructureHandler::get_LastStructureId()<<endl;
        ...
        // TODO replace MULTIMEDIA_DOCUMENT_READER0 with consistent value
        std::string MULTIMEDIA_DOCUMENT_READER0("MULTIMEDIA_DOCUMENT_READER0");
        Node node( MULTIMEDIA_DOCUMENT_READER0, handler->StructureHandler::get_LastStructureId(), 0, 0, 0, "", "xml", "", "hierarchy", false, false, false, false, 0, 0, 0, 0);
...
//         m_content_id++;
    ...
    case BIN_NUM_BLOC:
    {
        ...
        document.setIntValue("ContentId",handler->StructureHandler::get_LastContentId());
        ...
        handler->processSBinNum(vect,name,handler->StructureHandler::get_LastContentId());        
        ...
        Node node( MULTIMEDIA_DOCUMENT_READER2, handler->StructureHandler::get_LastStructureId(),0, handler->StructureHandler::get_LastContentId(), 2, path, "ima", "", name, false, false, false, false, 0, 0, 0, 0);
        handler->addContentNode(node);
    ...
    case BOW_TEXT_BLOC:
    {
        ...
        handler->processBoWText(&document,handler->StructureHandler::get_LastContentId());
        // Node ( STRUCT_ID structId, NODE_ID nodeId, CONTENT_ID contentId, int indexid , std::string uri, std::string nodeMedia, std::string nodeType, std::string descrId , bool isRoot, bool isLeaf, bool isFile, bool isMultimedia, TOPO_POS nodeStart, TOPO_POS nodeEnd, TOPO_POS nodeLength, NODE_ID nodeParent )
        ...
        Node node( MULTIMEDIA_DOCUMENT_READER3, handler->StructureHandler::get_LastStructureId(), 0, handler->StructureHandler::get_LastContentId(), 1, "", "text", "", "bow", false, false, false, false, 0, 0, 0, 0);
        handler->addContentNode(node);
        handler->StructureHandler::set_LastContentId(handler->StructureHandler::get_LastContentId()+1);
        break;
    }
    case NODE_PROPERTIES_BLOC:
    {
        ...
        handler->ContentHandler <Common::Misc::GenericDocumentProperties >::addContent(handler->lastNode(),document);
        break;
    }
    case END_BLOC:
    {
...
void MultimediaXMLStructureReader::
processSBinNum(const std::vector< float >& vect, const string& SID, int CID)
{
  LIMA_UNUSED(SID);
