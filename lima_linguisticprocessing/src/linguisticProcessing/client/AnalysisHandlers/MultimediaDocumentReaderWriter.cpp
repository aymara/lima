...
{
  Lima::Common::BagOfWords::BoWXMLWriter::setSpaces(s);
  m_dmult->setSpaces(s);
}

void MultimediaXMLWriter::incIndent(){m_dmult->incIndent();Lima::Common::BagOfWords::BoWXMLWriter::incIndent();}
void MultimediaXMLWriter::decIndent(){m_dmult->decIndent();Lima::Common::BagOfWords::BoWXMLWriter::decIndent();}

void MultimediaXMLWriter::openSNode( 
  const Lima::Common::Misc::GenericDocumentProperties* /*unused properties*/,
  const std::string& elementName ) {
  m_dmult->m_outputStream << m_dmult->m_spaces << "<node elementName=\"" << elementName << "\">" << std::endl;
  incIndent();
}
...
void MultimediaXMLWriter::openSIndexingNode(
  const Lima::Common::Misc::GenericDocumentProperties* /*unused properties*/,
  const std::string& elementName ) {
  m_dmult->m_outputStream << m_dmult->m_spaces << "<node elementName=\"" << elementName << "\" indexingNode=\"yes\">" << std::endl;
  incIndent();
}
...
//***********************************************************************
// reader
MultimediaBinaryReader::MultimediaBinaryReader():Lima::Common::BagOfWords::BoWBinaryReader()
{m_content_id=1;}
...
void MultimediaBinaryReader::readBinNum(std::istream& file,unsigned int& cid,string& path,string& name,std::vector<float>& vect)
{
...
void MultimediaBinaryReader::readMultimediaDocumentBlock(std::istream& file,
...
void MultimediaXMLWriter::processSContent( const Lima::Common::Misc::GenericDocumentProperties* properties )
{
  m_dmult->m_outputStream << m_dmult->m_spaces << "<content type=\"" << properties->getStringValue("type").first << "\">" << std::endl;
  incIndent();  
}
...
void  MultimediaXMLWriterPrivate::writeVectorFloat(std::vector<float> vect)
{
  ...
  m_outputStream<<m_spaces<<"<vector type=\"float\" n=\""<<size<<"\">"<<std::endl;
