...
AnnotationData::~AnnotationData()
{
  for (std::map< uint64_t, const Dumper* >::const_iterator it=m_dumpFunctions.begin();
...
AnnotationGraphEdge AnnotationData::createAnnotationEdge(AnnotationGraphVertex s, AnnotationGraphVertex t)
{
  ...
  return add_edge(s, t, m_graph).first;
}
...
void AnnotationData::annotate(AnnotationGraphVertex vs, AnnotationGraphVertex vt, const LimaString& annot, uint64_t value)
{
  ...
  boost::tie(e, ok) = edge(vs,vt,m_graph);
  if (!ok) e = createAnnotationEdge(vs, vt);
  annotate(e, m_pool[annot], value);
}
...
{
  const uint64_t idx = const_cast<AnnotationData*>(this)->m_pool[annot];
...
{
  return stringAnnotation(v, const_cast<AnnotationData*>(this)->m_pool[annot]);
}

const GenericAnnotation& AnnotationData::annotation(AnnotationGraphVertex v, const LimaString& annot) const
{
  return annotation(v, const_cast<AnnotationData*>(this)->m_pool[annot]);
}
...
{
  return stringAnnotation(v1, v2, const_cast<AnnotationData*>(this)->m_pool[annot]);
}
...
{
  return annotation(v1, v2, const_cast<AnnotationData*>(this)->m_pool[annot]);
}
...
{
  if (m_dumpFunctions.find(annot) == m_dumpFunctions.end())
  {
    m_dumpFunctions[annot] = new DummyDumpFunction();
  }
  VertexAGIannotPropertyMap map = get(vertex_iannot, m_graph);
  map[v][annot] = value;
  AGLOGINIT;
...
void AnnotationData::annotate(AnnotationGraphVertex v, uint64_t annot, const GenericAnnotation& value)
{
  ...
  VertexAGGannotPropertyMap map = get(vertex_gannot, m_graph);
  if (num_vertices(m_graph) <= v)
  {
    std::ostringstream oss;
    oss << "Trying to annotate an inexisting vertex: " << v
    << ". Graph size in vertices is: " << num_vertices(m_graph) << std::endl;
    throw std::range_error(oss.str());
  }
  std::map< uint64_t, GenericAnnotation > m = map[v];
  map[v][annot] = value;
  AGLOGINIT;
  LDEBUG << "Annotating " << v << " ("<< m_pool[StringsPoolIndex(annot)] <<") : GA";
}
...
void AnnotationData::annotate(AnnotationGraphVertex vs, AnnotationGraphVertex vt, uint64_t annot, uint64_t value)
{
  ...
  AGLOGINIT;
  LDEBUG << "Annotating " << vs << " -> " << vt << " ("<<m_pool[StringsPoolIndex(annot)]<<") : " << value;
}
...
void AnnotationData::annotate(AnnotationGraphEdge e, uint64_t annot, uint64_t value)
{
  ...
  EdgeAGIannotPropertyMap map = get(edge_iannot, m_graph);
  map[e][annot] = value;
}
...
void AnnotationData::annotate(AnnotationGraphEdge e, uint64_t annot, const LimaString& value)
{
  ...
  EdgeAGSannotPropertyMap map = get(edge_sannot, m_graph);
  map[e][annot] = value;
}
...
void AnnotationData::annotate(AnnotationGraphEdge e, uint64_t annot, const GenericAnnotation& value)
{
  ...
  EdgeAGGannotPropertyMap map = get(edge_gannot, m_graph);
  map[e][annot] = value;
}
...
{
  VertexAGIannotPropertyMap map = get(vertex_iannot, const_cast<AnnotationData*>(this)->m_graph);
  return map[v][annot];
}

const LimaString& AnnotationData::stringAnnotation(AnnotationGraphVertex v, uint64_t annot) const
{
  VertexAGSannotPropertyMap map = get(vertex_sannot, const_cast<AnnotationData*>(this)->m_graph);
  return map[v][annot];
}

const GenericAnnotation& AnnotationData::annotation(AnnotationGraphVertex v, uint64_t annot) const
{
  VertexAGGannotPropertyMap map = get(vertex_gannot, const_cast<AnnotationData*>(this)->m_graph);
  return map[v][annot];
}
...
{
  CEdgeAGGannotPropertyMap map = get(edge_gannot, m_graph);
...
{
  bool ok; AnnotationGraphEdge e;
  boost::tie(e, ok) = edge(v1,v2,m_graph);
  if (!ok)
  {
    std::ostringstream oss;
  ...
  CEdgeAGIannotPropertyMap map = get(edge_iannot, m_graph);
  return (*(map[e].find(annot))).second;
}
...
const LimaString& AnnotationData::stringAnnotation(AnnotationGraphVertex v1, AnnotationGraphVertex v2, uint64_t annot) const
{
  ...
  CEdgeAGSannotPropertyMap map = get(edge_sannot, m_graph);
  return (*(map[e].find(annot))).second;
}
...
void AnnotationData::dumpFunction(const std::string& annot, const AnnotationData::Dumper* f)
{
  ...
  if (m_dumpFunctions.find(annotId) != m_dumpFunctions.end())
  {
    AGLOGINIT;
    LWARN << "Replacing dump function for " << annot << " annotation. Deleting old one";
    delete m_dumpFunctions[annotId];
  }
  m_dumpFunctions[annotId] = f;
}
                                 ...
                                 const std::string& second, AnnotationGraphVertex secondVx)
{
  ...
  addMatching( m_pool[ Misc::utf8stdstring2limastring(first + second) ], firstVx, secondVx);
  addMatching( m_pool[ Misc::utf8stdstring2limastring(second + first) ], secondVx, firstVx);
}
...
std::set< AnnotationGraphVertex > AnnotationData::matches(const std::string& first, AnnotationGraphVertex firstVx,
  ...
  {
    return matches( m_pool[ Misc::utf8stdstring2limastring(first + second) ], firstVx);
  }
...
{
  return isMatching( m_pool[ Misc::utf8stdstring2limastring(first + second) ], firstVx, secondVx);
}
...
{
  addMatching( m_pool[ m_pool[first] + m_pool[second] ], firstVx, secondVx);
  addMatching( m_pool[ m_pool[second] + m_pool[first] ], secondVx, firstVx);
}
...
std::set< AnnotationGraphVertex > AnnotationData::matches(const StringsPoolIndex& first, AnnotationGraphVertex firstVx,
  ...
  {
    return matches( m_pool[ m_pool[first] + m_pool[second] ], firstVx);
  }
...
std::set< AnnotationGraphVertex > AnnotationData::matches(const StringsPoolIndex& direction,
      AnnotationGraphVertex firstVx) const
  {
    ...
    else
    {
      ...
      range = const_cast< std::map< StringsPoolIndex, std::multimap<AnnotationGraphVertex, AnnotationGraphVertex> >& >(m_matchings)[direction].equal_range(firstVx);
                                      ...
                                      std::set< std::string >& excepted)
{
  ...
  for (; eit!=eit_end; eit++)
  {
    iexcepted.insert(m_pool[Misc::utf8stdstring2limastring(*eit)]);
  }

  VertexAGIannotPropertyMap iannotsMap = get(vertex_iannot, m_graph);
  ...
  VertexAGSannotPropertyMap sannotsMap = get(vertex_sannot, m_graph);
  ...
  VertexAGGannotPropertyMap gannotsMap = get(vertex_gannot, m_graph);
